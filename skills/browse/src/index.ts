...
async function isDaemonRunning(): Promise<boolean> {
  try {
    const res = await fetch(`http://localhost:${PORT}`, { method: "POST", body: JSON.stringify({ cmd: "status", args: [] }), headers: { "Content-Type": "application/json" }, signal: AbortSignal.timeout(1000) });
    return res.ok;
  } catch {
    return false;
  }
